...
#
# Self-Play RL fine-tuning of CTM Chess Engine on
# Google Cloud TPU (4 cores).
# Requirements:
...
# Suppress the absl timezone banner
...
def train_fn(index, flags=None):
    ...
    # Resume from checkpoint (CPU load → broadcast → TPU copy)
    ...
    if ckpt:
        ...
        if "optimizer_state_dict" in data:
            ...
            # Move optimizer state tensors onto TPU
        ...
        start_iter = (
            int(
                re.search(r"ctm_rl_ckpt_(\d+)\.pt", ckpt)
                .group(1)
            ) + 1
        )
    ...
    for it in range(start_iter, RL_ITERS+1):
        ...
        for _ in range(BATCH_SIZE):
            ...
            while (not board.is_game_over(claim_draw=True)
                ...
                probs = F.softmax(
                    (step_logits + mask) / TEMPERATURE, dim=-1
                )
        ...
        loss = sum(-lp * (r - baseline)
                   for lp, r in zip(logps, R)) / len(games)
