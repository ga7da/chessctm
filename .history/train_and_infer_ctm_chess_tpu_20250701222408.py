...
def train_fn(index, flags=None):
    ...
    model = ContinuousThoughtMachine(
        ...
        backbone_type="mlp",            # use a real backbone so weights ≠ 0
    ...
    for it in range(start_iter, RL_ITERS + 1):
        ...
        for _ in range(BATCH_SIZE):
            ...
            while (not board.is_game_over(claim_draw=True)
                ...
                probs = F.softmax((step_logits + mask) / TEMPERATURE,
                                  dim=-1)
        ...
        R_tensor = torch.tensor(returns, device=device)
        baseline = R_tensor.mean().detach()
        loss = sum(-lp * (r - baseline)
                   for lp, r in zip(logps, R_tensor)) / len(games)
